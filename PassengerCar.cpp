...
PassengerCar::PassengerCar()
{
	...
	productionDate = getInput(i_eg, "Year of production:", 1800, getYear(), 40);
	bodyType = getInput(s_eg, "Body style:", "@");
	numOfPassengers = getInput(i_eg, "Number of passengers:", 1, 9, 40);
	numOfDoors = getInput(i_eg, "Number of doors:", 2, 5, 40);
...
PassengerCar::PassengerCar(char * data[])
{
	brand = (string)data[1];
	model = (string)data[2];
	...
	productionDate = stoi(data[10]);
	bodyType = (string)data[11];
	numOfPassengers = stoi(data[12]);
	numOfDoors = stoi(data[13]);
}
...
void PassengerCar::showData(bool showWidthDetails)
{
	cout.width(30);
	cout << left << "Brand:";
	...
	if (showWidthDetails) {
		...
		cout.width(30);
		cout << left << "Year of production:";
		cout << this->getProductionDate() << endl;

		cout.width(30);
		cout << left << "Body style:";
		cout << this->getBodyType() << endl;

		cout.width(30);
		cout << left << "Number of passengers:";
		cout << this->getNumOfPassengers() << endl;

		cout.width(30);
		cout << left << "Number of doors:";
		cout << this->getNumOfDoors() << endl;
	}
...
void PassengerCar::saveData(ofstream &file)
{
	file << "p;" << this->getBrand() << ";" << this->getModel() << ";" << this->getEnginePower() << ";" <<
		this->getEngineCapacity() << ";" << this->getFuelUsage() << ";" << this->getGearboxType() << ";" <<
		this->getMileage() << ";" << this->getCost() << ";" << this->getAvailability() << ";" <<
		this->getProductionDate() << ";" << this->getBodyType() << ";" << this->getNumOfPassengers() << ";" <<
		this->getNumOfDoors() << endl;
}
